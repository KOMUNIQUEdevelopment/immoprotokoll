  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-black/10 bg-white">
      <div className="container mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
        <Link href={`/${lang}`} className="font-bold text-xl tracking-tight uppercase">
          ImmoProtokoll
        </Link>
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <a 
            ...
            className="bg-black text-white px-5 py-2 text-sm font-bold hover:bg-black/80 transition-colors"
          >
            {t.nav.start_free}
          </a>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-16 left-0 w-full bg-white border-b border-black/10 p-4 flex flex-col gap-4 shadow-xl">
          ...
          <a 
            href="https://app.immoprotokoll.com" 
            className="bg-black text-white text-center py-3 mt-2 font-bold w-full"
          >
            {t.nav.start_free}
          </a>
        </div>
